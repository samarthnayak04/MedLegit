...
const Bubbles = ({ bubbleCount = 50, colors = ["#0b0ec6ff", "#06b6d4", "#fff"] }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < bubbleCount; i++) {
      ...
      bubbles.push({ element: bubble, speed: Math.random() * 2 + 1, sway: Math.random() * 50 - 25 });
    }
    ...
    const animate = () => {
      bubbles.forEach(bubble => {
        ...
        bubble.element.style.left = `calc(${bubble.element.style.left} + ${Math.sin(bottom / 50) * bubble.sway}px)`;

        if (bottom > window.innerHeight + 50) {
          bubble.element.style.bottom = `-${parseFloat(bubble.element.style.height)}px`;
...
// ParticleText Component (Updated with textured mask effect)
const ParticleText = ({ text = "MedLegit" }) => (
  <motion.h2
    initial={{ opacity: 0, scale: 0.8 }}
    animate={{ opacity: 1, scale: 1 }}
    transition={{ duration: .5, ease: 'easeOut' }}
    className="text-[8rem] md:text-[12rem] font-extrabold uppercase text-transparent bg-clip-text relative z-10 text-center"
    style={{
      backgroundImage:
...
export default function LandingPage() {
  ...
  <DecryptingText targetText="Your trusted platform for secure and efficient medical-legal solutions." speed={3} />
      ...
      <div className="relative text-left z-10 px-8 py-10 rounded-2xl w-full bg-white dark:bg-black shadow-2xl ring-1 ring-cyan-400/40">
        ...
        <div className="mt-10 space-y-3 text-gray-700 dark:text-gray-200">
          ...
          <p>
            ...
            , and actionable insights for healthcare and legal professionals.
          </p>
          <p>
        ...
        <div className="mt-6 flex justify-center">
          <ShimmerButton onClick={() => navigate("/signup")} text="Sign Up" />
        </div>
      </div>
    </motion.div>
  </div>
